package com.wadpam.gaelic.oauth.provider.dao;

import java.io.Serializable;
import java.util.ArrayList;
import java.util.Collection;
import java.util.Date;
import java.util.List;
import java.util.Map;
import java.util.Properties;
import java.util.TreeMap;
import org.xml.sax.ContentHandler;
import org.xml.sax.SAXException;
import net.sf.mardao.core.CursorPage;
import net.sf.mardao.core.Filter;
import net.sf.mardao.core.dao.DaoImpl;
import net.sf.mardao.core.dao.TypeDaoImpl;
import net.sf.mardao.core.geo.DLocation;
import com.wadpam.gaelic.oauth.provider.domain.Do2pToken;

/**
 * The Do2pToken domain-object specific finders and methods go in this POJO.
 * 
 * Generated on 2013-07-18T12:02:44.205+0700.
 * @author mardao DAO generator (net.sf.mardao.plugin.ProcessDomainMojo)
 */
public class GeneratedDo2pTokenDaoImpl extends TypeDaoImpl<Do2pToken, java.lang.Long> 
	implements GeneratedDo2pTokenDao {


    /** to list the property names for ManyToOne relations */
    @Override
    protected List<String> getBasicColumnNames() {
        return BASIC_NAMES;
    }

    /** to list the property names for ManyToOne relations */
    @Override
    protected List<String> getManyToOneColumnNames() {
        return MANY_TO_ONE_NAMES;
    }

    private final Map<String, DaoImpl> MANY_TO_ONE_DAOS = new TreeMap<String, DaoImpl>();

    /** Default constructor */
   public GeneratedDo2pTokenDaoImpl() {
      super(Do2pToken.class, java.lang.Long.class);
   }

   // ------ BEGIN DaoImpl overrides -----------------------------
   
   public String getPrimaryKeyColumnName() {
   		return COLUMN_NAME_ID;
   }
   
   public List<String> getColumnNames() {
        return COLUMN_NAMES;
   }

   @Override
   protected DaoImpl getManyToOneDao(String columnName) {
       return MANY_TO_ONE_DAOS.get(columnName);
   }

    @Override
    protected Object getDomainProperty(Do2pToken domain, String name) {
        Object value;
        // simple key?
        if (COLUMN_NAME_ID.equals(name)) {
            value = domain.getId();
        }
        // parent key?
        else if (COLUMN_NAME_PROFILEKEY.equals(name)) {
            value = domain.getProfileKey();
        }
        // fields
        else if (COLUMN_NAME_ACCESSTOKEN.equals(name)) {
            value = domain.getAccessToken();
        }
        else if (COLUMN_NAME_CLIENTID.equals(name)) {
            value = domain.getClientId();
        }
        else if (COLUMN_NAME_CREATEDBY.equals(name)) {
            value = domain.getCreatedBy();
        }
        else if (COLUMN_NAME_CREATEDDATE.equals(name)) {
            value = domain.getCreatedDate();
        }
        else if (COLUMN_NAME_EXPIRYDATE.equals(name)) {
            value = domain.getExpiryDate();
        }
        else if (COLUMN_NAME_REFRESHTOKEN.equals(name)) {
            value = domain.getRefreshToken();
        }
        else if (COLUMN_NAME_SCOPE.equals(name)) {
            value = domain.getScope();
        }
        else if (COLUMN_NAME_UPDATEDBY.equals(name)) {
            value = domain.getUpdatedBy();
        }
        else if (COLUMN_NAME_UPDATEDDATE.equals(name)) {
            value = domain.getUpdatedDate();
        }
        // one-to-ones
        // many-to-ones
        // many-to-manys
        else {
            value = super.getDomainProperty(domain, name);
        }

        return value;
    }

    /**
     * Returns the class of the domain property for specified column
     * @param name
     * @return the class of the domain property
     */
    public Class getColumnClass(String name) {
        Class clazz;
        // simple key?
        if (COLUMN_NAME_ID.equals(name)) {
            clazz = java.lang.Long.class;
        }
        // parent key?
        else if (COLUMN_NAME_PROFILEKEY.equals(name)) {
            clazz = java.lang.Object.class;
        }
        // fields
        else if (COLUMN_NAME_ACCESSTOKEN.equals(name)) {
            clazz = java.lang.String.class;
        }
        else if (COLUMN_NAME_CLIENTID.equals(name)) {
            clazz = java.lang.Long.class;
        }
        else if (COLUMN_NAME_CREATEDBY.equals(name)) {
            clazz = java.lang.String.class;
        }
        else if (COLUMN_NAME_CREATEDDATE.equals(name)) {
            clazz = java.util.Date.class;
        }
        else if (COLUMN_NAME_EXPIRYDATE.equals(name)) {
            clazz = java.util.Date.class;
        }
        else if (COLUMN_NAME_REFRESHTOKEN.equals(name)) {
            clazz = java.lang.String.class;
        }
        else if (COLUMN_NAME_SCOPE.equals(name)) {
            clazz = java.lang.String.class;
        }
        else if (COLUMN_NAME_UPDATEDBY.equals(name)) {
            clazz = java.lang.String.class;
        }
        else if (COLUMN_NAME_UPDATEDDATE.equals(name)) {
            clazz = java.util.Date.class;
        }
        // one-to-ones
        // many-to-ones
        // many-to-manys
        else {
            throw new IllegalArgumentException("No such column " + name);
        }

        return clazz;
    }
      
    @Override
    protected void setDomainProperty(final Do2pToken domain, final String name, final Object value) {
        // simple key?
        if (COLUMN_NAME_ID.equals(name)) {
            domain.setId((java.lang.Long) value);
        }
        // parent key?
        else if (COLUMN_NAME_PROFILEKEY.equals(name)) {
            domain.setProfileKey((java.lang.Object) value);
        }
        // fields
        else if (COLUMN_NAME_ACCESSTOKEN.equals(name)) {
            domain.setAccessToken((java.lang.String) value);
        }
        else if (COLUMN_NAME_CLIENTID.equals(name)) {
            domain.setClientId((java.lang.Long) value);
        }
        else if (COLUMN_NAME_CREATEDBY.equals(name)) {
            domain.setCreatedBy((java.lang.String) value);
        }
        else if (COLUMN_NAME_CREATEDDATE.equals(name)) {
            domain.setCreatedDate((java.util.Date) value);
        }
        else if (COLUMN_NAME_EXPIRYDATE.equals(name)) {
            domain.setExpiryDate((java.util.Date) value);
        }
        else if (COLUMN_NAME_REFRESHTOKEN.equals(name)) {
            domain.setRefreshToken((java.lang.String) value);
        }
        else if (COLUMN_NAME_SCOPE.equals(name)) {
            domain.setScope((java.lang.String) value);
        }
        else if (COLUMN_NAME_UPDATEDBY.equals(name)) {
            domain.setUpdatedBy((java.lang.String) value);
        }
        else if (COLUMN_NAME_UPDATEDDATE.equals(name)) {
            domain.setUpdatedDate((java.util.Date) value);
        }
        // one-to-ones
        // many-to-ones
        // many-to-manys
        else {
            super.setDomainProperty(domain, name, value);
        }
    }

    @Override
    protected void setDomainStringProperty(final Do2pToken domain, final String name, final Map<String, String> properties) {
        final String value = properties.get(name);
        Class clazz = getColumnClass(name);
        // many-to-ones
        setDomainProperty(domain, name, parseProperty(value, clazz));
    }

    /**
     * Overrides to substitute Entity properties with foreign keys
     */
    @Override
    protected void setCoreProperty(Object core, String name, Object value) {
        if (null == core || null == name) {
            return;
        }
        else if (null == value) {
            // do nothing in particular, will call super at end
        }
        super.setCoreProperty(core, name, value);
    }

    /** Default implementation returns null, overrides for profileKey parent */
    public String getParentKeyColumnName() {
        return COLUMN_NAME_PROFILEKEY;
    }

   // ------ END DaoImpl overrides -----------------------------

        public Object getParentKey(Do2pToken domain) {
            return domain.getProfileKey();
        }

        public void setParentKey(Do2pToken domain, Object profileKey) {
            domain.setProfileKey((Serializable) profileKey);
        }

	// ----------------------- parent finders -------------------------------

	/**
	 * query-by method for parent field profileKey
	 * @param profileKey the specified attribute
	 * @return an Iterable of Do2pTokens for the specified parent
	 */
	public final Iterable<Do2pToken> queryByProfileKey(Object profileKey) {
            return queryIterable(false, 0, -1, profileKey, null, null, false, null, false);
	}
	
	/**
	 * query-key-by method for parent field profileKey
	 * @param profileKey the parent
	 * @return an Iterable of keys to the Do2pTokens with the specified parent
	 */
	public final Iterable<java.lang.Long> queryKeysByProfileKey(Object profileKey) {
            return queryIterableKeys(0, -1, profileKey, null, null, false, null, false);
	}

	/**
	 * query-page-by method for parent field profileKey
	 * @param profileKey the specified parent
         * @param pageSize the number of domain entities in the page
         * @param cursorString non-null if get next page
	 * @return a Page of Do2pTokens for the specified profileKey
	 */
	public final CursorPage<Do2pToken> queryPageByProfileKey(java.lang.Object profileKey,
            int pageSize, String cursorString) {
            return queryPage(false, pageSize, profileKey, null, null, false, null, false, cursorString);
        }


        /**
         * @return the simple key for specified Do2pToken domain object
         */
        public Long getSimpleKey(Do2pToken domain) {
            if (null == domain) {
                return null;
            }
            return domain.getId();
        }

        /**
         * @return the simple key for specified Do2pToken domain object
         */
        public void setSimpleKey(Do2pToken domain, Long id) {
            domain.setId(id);
        }

        public String getCreatedByColumnName() {
            return COLUMN_NAME_CREATEDBY;
        }

        public String getCreatedBy(Do2pToken domain) {
            if (null == domain) {
                return null;
            }
            return domain.getCreatedBy();
        }

        public void _setCreatedBy(Do2pToken domain, String creator) {
            domain.setCreatedBy(creator);
        }

        public String getUpdatedByColumnName() {
            return COLUMN_NAME_UPDATEDBY;
        }

        public String getUpdatedBy(Do2pToken domain) {
            if (null == domain) {
                return null;
            }
            return domain.getUpdatedBy();
        }

        public void _setUpdatedBy(Do2pToken domain, String updator) {
            domain.setUpdatedBy(updator);
        }

        public String getCreatedDateColumnName() {
            return COLUMN_NAME_CREATEDDATE;
        }

        public Date getCreatedDate(Do2pToken domain) {
            if (null == domain) {
                return null;
            }
            return domain.getCreatedDate();
        }

        public void _setCreatedDate(Do2pToken domain, Date date) {
            domain.setCreatedDate(date);
        }

        public String getUpdatedDateColumnName() {
            return COLUMN_NAME_UPDATEDDATE;
        }

        public Date getUpdatedDate(Do2pToken domain) {
            if (null == domain) {
                return null;
            }
            return domain.getUpdatedDate();
        }

        public void _setUpdatedDate(Do2pToken domain, Date date) {
            domain.setUpdatedDate(date);
        }

	// ----------------------- field finders -------------------------------
	/**
	 * find-by method for unique attribute field accessToken
	 * @param accessToken the unique attribute
	 * @return the unique Do2pToken for the specified attribute
	 */
	public final Do2pToken findByAccessToken(java.lang.String accessToken) {
                Filter filter = createEqualsFilter(COLUMN_NAME_ACCESSTOKEN, accessToken);
		return findUniqueBy(filter);
	}

	/**
	 * find-key-by method for unique attribute field accessToken
	 * @param accessToken the unique attribute
	 * @return the unique Do2pToken for the specified attribute
	 */
	public final java.lang.Long findKeyByAccessToken(java.lang.String accessToken) {
                Filter filter = createEqualsFilter(COLUMN_NAME_ACCESSTOKEN, accessToken);
		return findUniqueKeyBy(filter);
	}
	 
	/**
         * {@inheritDoc}
	 */
	public final Iterable<Do2pToken> queryByClientId(java.lang.Long clientId) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_CLIENTID, clientId);
            return queryIterable(false, 0, -1, null, null, null, false, null, false, filter);
	}
	
	/**
	 * query-key-by method for attribute field clientId
	 * @param clientId the specified attribute
	 * @return an Iterable of keys to the Do2pTokens with the specified attribute
	 */
	public final Iterable<java.lang.Long> queryKeysByClientId(java.lang.Long clientId) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_CLIENTID, clientId);
            return queryIterableKeys(0, -1, null, null, null, false, null, false, filter);
	}

	/**
	 * query-page-by method for field clientId
	 * @param clientId the specified attribute
         * @param pageSize the number of domain entities in the page
         * @param cursorString non-null if get next page
	 * @return a Page of Do2pTokens for the specified clientId
	 */
	public final CursorPage<Do2pToken> queryPageByClientId(java.lang.Long clientId,
            int pageSize, String cursorString) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_CLIENTID, clientId);
            return queryPage(false, pageSize, null, null, null, false, null, false, cursorString, filter);
        }

	 
	/**
         * {@inheritDoc}
	 */
	public final Iterable<Do2pToken> queryByCreatedBy(java.lang.String createdBy) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_CREATEDBY, createdBy);
            return queryIterable(false, 0, -1, null, null, null, false, null, false, filter);
	}
	
	/**
	 * query-key-by method for attribute field createdBy
	 * @param createdBy the specified attribute
	 * @return an Iterable of keys to the Do2pTokens with the specified attribute
	 */
	public final Iterable<java.lang.Long> queryKeysByCreatedBy(java.lang.String createdBy) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_CREATEDBY, createdBy);
            return queryIterableKeys(0, -1, null, null, null, false, null, false, filter);
	}

	/**
	 * query-page-by method for field createdBy
	 * @param createdBy the specified attribute
         * @param pageSize the number of domain entities in the page
         * @param cursorString non-null if get next page
	 * @return a Page of Do2pTokens for the specified createdBy
	 */
	public final CursorPage<Do2pToken> queryPageByCreatedBy(java.lang.String createdBy,
            int pageSize, String cursorString) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_CREATEDBY, createdBy);
            return queryPage(false, pageSize, null, null, null, false, null, false, cursorString, filter);
        }

	 
	/**
         * {@inheritDoc}
	 */
	public final Iterable<Do2pToken> queryByCreatedDate(java.util.Date createdDate) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_CREATEDDATE, createdDate);
            return queryIterable(false, 0, -1, null, null, null, false, null, false, filter);
	}
	
	/**
	 * query-key-by method for attribute field createdDate
	 * @param createdDate the specified attribute
	 * @return an Iterable of keys to the Do2pTokens with the specified attribute
	 */
	public final Iterable<java.lang.Long> queryKeysByCreatedDate(java.util.Date createdDate) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_CREATEDDATE, createdDate);
            return queryIterableKeys(0, -1, null, null, null, false, null, false, filter);
	}

	/**
	 * query-page-by method for field createdDate
	 * @param createdDate the specified attribute
         * @param pageSize the number of domain entities in the page
         * @param cursorString non-null if get next page
	 * @return a Page of Do2pTokens for the specified createdDate
	 */
	public final CursorPage<Do2pToken> queryPageByCreatedDate(java.util.Date createdDate,
            int pageSize, String cursorString) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_CREATEDDATE, createdDate);
            return queryPage(false, pageSize, null, null, null, false, null, false, cursorString, filter);
        }

	 
	/**
         * {@inheritDoc}
	 */
	public final Iterable<Do2pToken> queryByExpiryDate(java.util.Date expiryDate) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_EXPIRYDATE, expiryDate);
            return queryIterable(false, 0, -1, null, null, null, false, null, false, filter);
	}
	
	/**
	 * query-key-by method for attribute field expiryDate
	 * @param expiryDate the specified attribute
	 * @return an Iterable of keys to the Do2pTokens with the specified attribute
	 */
	public final Iterable<java.lang.Long> queryKeysByExpiryDate(java.util.Date expiryDate) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_EXPIRYDATE, expiryDate);
            return queryIterableKeys(0, -1, null, null, null, false, null, false, filter);
	}

	/**
	 * query-page-by method for field expiryDate
	 * @param expiryDate the specified attribute
         * @param pageSize the number of domain entities in the page
         * @param cursorString non-null if get next page
	 * @return a Page of Do2pTokens for the specified expiryDate
	 */
	public final CursorPage<Do2pToken> queryPageByExpiryDate(java.util.Date expiryDate,
            int pageSize, String cursorString) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_EXPIRYDATE, expiryDate);
            return queryPage(false, pageSize, null, null, null, false, null, false, cursorString, filter);
        }

	 
	/**
	 * find-by method for unique attribute field refreshToken
	 * @param refreshToken the unique attribute
	 * @return the unique Do2pToken for the specified attribute
	 */
	public final Do2pToken findByRefreshToken(java.lang.String refreshToken) {
                Filter filter = createEqualsFilter(COLUMN_NAME_REFRESHTOKEN, refreshToken);
		return findUniqueBy(filter);
	}

	/**
	 * find-key-by method for unique attribute field refreshToken
	 * @param refreshToken the unique attribute
	 * @return the unique Do2pToken for the specified attribute
	 */
	public final java.lang.Long findKeyByRefreshToken(java.lang.String refreshToken) {
                Filter filter = createEqualsFilter(COLUMN_NAME_REFRESHTOKEN, refreshToken);
		return findUniqueKeyBy(filter);
	}
	 
	/**
         * {@inheritDoc}
	 */
	public final Iterable<Do2pToken> queryByScope(java.lang.String scope) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_SCOPE, scope);
            return queryIterable(false, 0, -1, null, null, null, false, null, false, filter);
	}
	
	/**
	 * query-key-by method for attribute field scope
	 * @param scope the specified attribute
	 * @return an Iterable of keys to the Do2pTokens with the specified attribute
	 */
	public final Iterable<java.lang.Long> queryKeysByScope(java.lang.String scope) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_SCOPE, scope);
            return queryIterableKeys(0, -1, null, null, null, false, null, false, filter);
	}

	/**
	 * query-page-by method for field scope
	 * @param scope the specified attribute
         * @param pageSize the number of domain entities in the page
         * @param cursorString non-null if get next page
	 * @return a Page of Do2pTokens for the specified scope
	 */
	public final CursorPage<Do2pToken> queryPageByScope(java.lang.String scope,
            int pageSize, String cursorString) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_SCOPE, scope);
            return queryPage(false, pageSize, null, null, null, false, null, false, cursorString, filter);
        }

	 
	/**
         * {@inheritDoc}
	 */
	public final Iterable<Do2pToken> queryByUpdatedBy(java.lang.String updatedBy) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_UPDATEDBY, updatedBy);
            return queryIterable(false, 0, -1, null, null, null, false, null, false, filter);
	}
	
	/**
	 * query-key-by method for attribute field updatedBy
	 * @param updatedBy the specified attribute
	 * @return an Iterable of keys to the Do2pTokens with the specified attribute
	 */
	public final Iterable<java.lang.Long> queryKeysByUpdatedBy(java.lang.String updatedBy) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_UPDATEDBY, updatedBy);
            return queryIterableKeys(0, -1, null, null, null, false, null, false, filter);
	}

	/**
	 * query-page-by method for field updatedBy
	 * @param updatedBy the specified attribute
         * @param pageSize the number of domain entities in the page
         * @param cursorString non-null if get next page
	 * @return a Page of Do2pTokens for the specified updatedBy
	 */
	public final CursorPage<Do2pToken> queryPageByUpdatedBy(java.lang.String updatedBy,
            int pageSize, String cursorString) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_UPDATEDBY, updatedBy);
            return queryPage(false, pageSize, null, null, null, false, null, false, cursorString, filter);
        }

	 
	/**
         * {@inheritDoc}
	 */
	public final Iterable<Do2pToken> queryByUpdatedDate(java.util.Date updatedDate) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_UPDATEDDATE, updatedDate);
            return queryIterable(false, 0, -1, null, null, null, false, null, false, filter);
	}
	
	/**
	 * query-key-by method for attribute field updatedDate
	 * @param updatedDate the specified attribute
	 * @return an Iterable of keys to the Do2pTokens with the specified attribute
	 */
	public final Iterable<java.lang.Long> queryKeysByUpdatedDate(java.util.Date updatedDate) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_UPDATEDDATE, updatedDate);
            return queryIterableKeys(0, -1, null, null, null, false, null, false, filter);
	}

	/**
	 * query-page-by method for field updatedDate
	 * @param updatedDate the specified attribute
         * @param pageSize the number of domain entities in the page
         * @param cursorString non-null if get next page
	 * @return a Page of Do2pTokens for the specified updatedDate
	 */
	public final CursorPage<Do2pToken> queryPageByUpdatedDate(java.util.Date updatedDate,
            int pageSize, String cursorString) {
            final Filter filter = createEqualsFilter(COLUMN_NAME_UPDATEDDATE, updatedDate);
            return queryPage(false, pageSize, null, null, null, false, null, false, cursorString, filter);
        }

	 
	// ----------------------- one-to-one finders -------------------------

	// ----------------------- many-to-one finders -------------------------

	// ----------------------- many-to-many finders -------------------------

	// ----------------------- uniqueFields finders -------------------------

	// ----------------------- populate / persist method -------------------------

	/**
	 * Persist an entity given all attributes
	 */
	public Do2pToken persist(Object profileKey,  	
		java.lang.Long id, 
		java.lang.String accessToken, 
		java.lang.Long clientId, 
		java.util.Date expiryDate, 
		java.lang.String refreshToken, 
		java.lang.String scope) {

            Do2pToken domain = null;
            // if primaryKey specified, use it
            if (null != id) {
                    domain = findByPrimaryKey(profileKey, id);
            }
		
            // create new?
            if (null == domain) {
                    domain = new Do2pToken();
                    // set parent
                    domain.setProfileKey((java.lang.Object) profileKey);
                    // generate Id?
                    if (null != id) {
                            domain.setId(id);
                    }
                    // fields
                    domain.setAccessToken(accessToken);
                    domain.setClientId(clientId);
                    domain.setExpiryDate(expiryDate);
                    domain.setRefreshToken(refreshToken);
                    domain.setScope(scope);
                    // one-to-ones
                    // many-to-ones
			
                    persist(domain);
            }
            return domain;
	}


	/**
	 * Persists an entity unless it already exists
	 */
	public Do2pToken persist(java.lang.String accessToken, 
                java.lang.Long clientId, 
                java.util.Date expiryDate, 
                java.lang.String refreshToken, 
                java.lang.String scope) {
            Do2pToken domain = findByAccessToken(accessToken);
            if (null == domain) {
                domain = new Do2pToken();
                domain.setAccessToken(accessToken);
                domain.setClientId(clientId);
                domain.setExpiryDate(expiryDate);
                domain.setRefreshToken(refreshToken);
                domain.setScope(scope);
                persist(domain);
            }
            return domain;
	}


}
